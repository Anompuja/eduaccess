...
Future<void> showHeadmasterEditModal(
  ...
  return showDialog<void>(
    context: context,
    builder: (_) => HeadmasterEditModal(ref: ref, data: data),
  );
}

class HeadmasterEditModal extends ConsumerStatefulWidget {
  final WidgetRef ref;
  final HeadmasterRowData data;

  const HeadmasterEditModal({super.key, required this.ref, required this.data});

  @override
  ConsumerState<HeadmasterEditModal> createState() =>
      _HeadmasterEditModalState();
}

class _HeadmasterEditModalState extends ConsumerState<HeadmasterEditModal> {
  ...
  void initState() {
    ...
    _selectedGender = _normalizeGender(widget.data.gender);
  }
  ...
  Future<void> _pickBirthDate() async {
    final initialDate =
        DateTime.tryParse(_birthDateCtrl.text.trim()) ?? DateTime.now();
  ...
  Future<void> _saveHeadmaster() async {
    ...
    try {
      final headmaster = await ref.read(
        updateHeadmasterProvider((
          id: widget.data.headmasterId,
          data: {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${headmaster.name} berhasil diperbarui')),
        );
      }
    } catch (e) {
      if (mounted) {
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Perubahan akan dikirim langsung ke backend Headmaster.',
              ...
              Row(
                ...
                children: [
                  ...
                  AppButton.primary(
                    ...
                    onPressed: _isLoading ? null : _saveHeadmaster,
                  ),
                ],
              ),
            ],
